...
extern CDspSetRs232Dlg* pGlobalDlg;
...
CRs232::CRs232( )
{
	...
	memset(m_cRxBuffer, NULL, MAXBLOCK );
}
...
DWORD Rs232WatchProc(LPVOID lpData)
{
	...
	if ( os.hEvent == NULL )
	{
		MessageBox( NULL, "Failed to create event for thread!", "Rs232 Error!",
	...
	while (npRs232->m_bConnected)
	{
		...
		if ((dwEvtMask & EV_RXCHAR) == EV_RXCHAR)
		{
			...
			do
			{
				...
				{
					npRs232->SetReadData(InData, nLength);
					//이곳에서 데이타를 받는다.
				}
			}
			while ( nLength > 0 ) ;
...
//데이타를 일고 데이타를 읽었다는
//메세지를 리턴한다.
void CRs232::SetReadData(LPSTR data, int nLength)
{
	...
	//알려준다.
// 	SendMessage(m_hwnd, WM_RECEIVEDATA, 0, (LPARAM)nLength);
	pGlobalDlg->SendMessage(WM_RECEIVEDATA, 0, (LPARAM)nLength);
}
